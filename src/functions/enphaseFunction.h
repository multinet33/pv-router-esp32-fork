...
void Enphase_get_5(void)
{
  ...
  {
      Serial.println("Enphase_get_5 : Max retries atteint, requete abandonnée");
      return;
  }
...
bool Enphase_get_7_Production(void)
{
  ...
  if (httpenphase.begin(fullurl))
  {
    ...
    {
      Serial.println("[1.Enphase Get production] GET... failed, error: " + String(httpCode)+ " " + httpenphase.errorToString(httpCode));
      nbErreurGetJsonProd++;
      //httpenphase.end();
    }
    //httpenphase.end();
...
bool Enphase_get_7_JWT(void)
{
  ...
  if (httpenphase.begin(fullurl))
  {
    ...
    if (ENPHASE_DEBUG) {
      Serial.println("Retour HTTPCode sur GET() : " + httpenphase.errorToString(httpCode));   
      Serial.print("Réponse Headers :");
    ...
    if (httpCode > 0)
    {
      ...
      if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY)
      {
        retour = true;
        // Token valide
        Serial.println("JWT Enphase contrôle token : TOKEN VALIDE ");
        TockenValide = true;
        SessionId.clear();
        ...
        if (SessionId.indexOf("sessionId") < 0)
        {
          retour = false;
          SessionId.clear();
          Serial.println("JWT Enphase contrôle token : PAS DE SESSION ID !!!");
          Serial.println("JWT SessionId : " + SessionId);
        }
        else
        {
          ...
          {
            SessionId.remove(SessionId.indexOf(";"));
          }
          Serial.println("JWT Enphase contrôle token sessionID: " + SessionId);
        }
      }
      else
      {
    ...
    {
      Serial.println("JWT Connexion refusée - Error code : " + String(httpCode));
      nbErreurCtrlTocken++;
    }
  }
  else
  ...
  {
    Serial.println("JWT Trop d'erreur  : redemmarrage");
    ESP.restart();
  }
...
{  
  ...
  { 
    ...
    if (SessionId.isEmpty() || ((Enphase_get_7_Production() == false) && (nbErreurGetJsonProd > 10)))
