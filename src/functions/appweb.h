...
String oscilloscope()
{
  ...
  while (timer < oscillo_freqmesure)
  {
    ...
    temp = adc1_get_raw((adc1_channel_t)4);
    signe = adc1_get_raw((adc1_channel_t)5);
...
bool getServermode(String Servermode)
{
  ...
  {
    gDisplayValues.screenstate = !gDisplayValues.screenstate;
    return true;
  }
  if (Servermode == "Jeedom")
  {
    config.UseJeedom = !config.UseJeedom;
    return true;
  }
  if (Servermode == "Autonome")
  {
    config.autonome = !config.autonome;
    return true;
  }
  ...
  if (Servermode == "envoy")
  {
    configmodule.enphase_present = !configmodule.enphase_present;
    return true;
  }
  if (Servermode == "fronius")
  {
    configmodule.Fronius_present = !configmodule.Fronius_present;
    return true;
  }
  if (Servermode == "TRI")
  {
    config.Shelly_tri = !config.Shelly_tri;
    return true;
  }
